...
class DINOHead_Nuclei_MultiHead_Prompt(DeformableDETRHead_Nuclei):
    ...
    def __init__(self,
                 *args,
                 ...
                 num_cls_fcs: int = 0,
                 weight_classes = None,
                 contrastive_cfg: ConfigType = dict(
                    loss_cont_cfg = dict(
        ...
        # contrastive loss
        loss_cont_cfg = contrastive_cfg['loss_cont_cfg']
        self.temperature = contrastive_cfg['temperature']
        self.logit_scale = nn.Parameter(torch.ones([]) * np.log(1 / self.temperature))
        self.loss_cont = MODELS.build(loss_cont_cfg)
        self.queue_len = contrastive_cfg['queue_len']
        self.register_buffer('queue_query', torch.randn(self.embed_dims*contrastive_cfg['num_queries'], self.queue_len))
        self.queue_query = F.normalize(self.queue_query, dim=0)
        self.register_buffer('queue_query_ptr', torch.zeros(1, dtype=torch.long))

        self.register_buffer('queue_text', torch.randn(self.embed_dims*contrastive_cfg['num_queries'], self.queue_len))
        self.queue_text = F.normalize(self.queue_text, dim=0)
        self.register_buffer('queue_text_ptr', torch.zeros(1, dtype=torch.long))
    ...
    def _init_layers(self) -> None:
        ...
        for num_classes in self.num_classes:
            ...
            if self.num_cls_fcs:
                ...
                fc_cls.append(Linear(self.embed_dims, num_classes))
    ...
    def loss(self, hidden_states: Tensor, references: List[Tensor],
             ...
             query_token: Tensor, cls_text_x: Tensor) -> dict:
        ...
        loss_inputs = outs + (enc_outputs_class, enc_outputs_coord,
                              batch_gt_instances, batch_img_metas, dn_meta, None,
                              query_token, cls_text_x)
    ...
    def loss_by_feat(
        ...
        query_token: Tensor = None,
        cls_text_x: Tensor = None,
    ) -> Dict[str, Tensor]:
        """Loss function.
        ...
        if query_token is not None:
            loss_dict['loss_contrastive'] = \
            self.loss_contrastive(
                query_token.flatten(1), cls_text_x.flatten(1))
    ...
    def loss_contrastive_single(self, pos, neg) -> Tensor:
        N = pos.size(0)
        logits = torch.cat((pos, neg), dim=1)
        logits *= self.logit_scale
        labels = torch.zeros((N,), dtype=torch.long).to(pos.device)

        loss = self.loss_cont(logits, labels)
        return loss

    def loss_contrastive(self, query_token: Tensor, cls_text_x: Tensor) -> Tensor:
        # query_token, idx_unshuffle = batch_shuffle_ddp(query_token)
        query_token = F.normalize(query_token, dim=-1)
        # query_token = batch_unshuffle_ddp(query_token, idx_unshuffle)

        # cls_text_x, idx_unshuffle = batch_shuffle_ddp(cls_text_x)
        text_x = F.normalize(cls_text_x, dim=-1)
        # text_x = batch_unshuffle_ddp(text_x, idx_unshuffle)

        try :
            q_pos = torch.einsum('nc, nc -> n', [query_token, text_x]).unsqueeze(-1)
            q_neg = torch.einsum('nc, ck -> nk', [query_token, self.queue_text.clone().detach()])
            loss_query = self.loss_contrastive_single(q_pos, q_neg)

            t_pos = torch.einsum('nc, nc -> n', [text_x, query_token]).unsqueeze(-1)
            t_neg = torch.einsum('nc, ck -> nk', [text_x, self.queue_query.clone().detach()])
            loss_text = self.loss_contrastive_single(t_pos, t_neg)
        except:
            import pdb; pdb.set_trace()
            print(1)
        self._dequeue_and_enqueue(text_x, "text")
        self._dequeue_and_enqueue(query_token, "query")
        return loss_text+loss_query

    @torch.no_grad()
    def _dequeue_and_enqueue(self, keys: torch.Tensor, mode) -> None:
        """Update queue."""
        ...
        if mode == "text":
            ptr = int(self.queue_query_ptr)
            assert self.queue_len % batch_size == 0  # for simplicity

            # replace the keys at ptr (dequeue and enqueue)
            self.queue_query[:, ptr:ptr + batch_size] = keys.transpose(0, 1)
            ptr = (ptr + batch_size) % self.queue_len  # move pointer

            self.queue_query_ptr[0] = ptr

        elif mode == "query":
            ptr = int(self.queue_text_ptr)
            assert self.queue_len % batch_size == 0  # for simplicity

            # replace the keys at ptr (dequeue and enqueue)
            self.queue_text[:, ptr:ptr + batch_size] = keys.transpose(0, 1)
            ptr = (ptr + batch_size) % self.queue_len  # move pointer

            self.queue_text_ptr[0] = ptr
