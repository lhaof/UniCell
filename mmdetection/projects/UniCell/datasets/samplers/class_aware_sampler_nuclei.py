...
class ClassAwareSampler_Nuclei(Sampler):
...
@DATA_SAMPLERS.register_module()
class DatasetAwareSampler_Nuclei(Sampler):
    r"""Sampler that restricts data loading to the label of the dataset.
    ...
    def __init__(self,
        ...
        self.num_sample_class = num_sample_class
        # Get per-label image list from dataset
        self.dataset_dict = self.get_data2imgs()

        self.num_samples = int(math.ceil(len(self.dataset) * 1.0 / world_size))
        self.total_size = self.num_samples * self.world_size

        # get number of images containing each dataset
        self.num_dataset_imgs = [len(x) for x in self.dataset_dict.values()]
        self.valid_dataset_inds = [
            i for i, length in enumerate(self.num_dataset_imgs) if length != 0
        ]
        self.num_datasets = len(self.valid_dataset_inds)

    def get_data2imgs(self) -> Dict[int, list]:
        datasets = self.dataset.metainfo.get('datasets', None)
        if datasets is None:
            raise ValueError('dataset metainfo must contain `datasets`')
        data2imgs = {i: [] for i in range(len(datasets))}
        for i in range(len(self.dataset)):
            data_ids = set([self.dataset.get_dataset_ids(i)])
            for data_id in data_ids:
                data2imgs[data_id].append(i)
        return data2imgs
    ...
    def __iter__(self) -> Iterator[int]:
        ...
        # initialize label list
        label_iter_list = RandomCycleIter(self.valid_dataset_inds, generator=g)
        # initialize each per-label image list
        data_iter_dict = dict()
        for i in self.valid_dataset_inds:
            data_iter_dict[i] = RandomCycleIter(self.dataset_dict[i], generator=g)
        ...
        # deterministically shuffle based on epoch
        num_bins = int(
            math.ceil(self.total_size * 1.0 / self.num_datasets /
                      self.num_sample_class))
        indices = []
        for i in range(num_bins):
            indices += gen_cat_img_inds(label_iter_list, data_iter_dict,
